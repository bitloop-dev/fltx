/**
 * fltx/detail/fdd_math_transcendental.h - dd transcendental math implementation details.
 *
 * dd cbrt, exp/log, pow, trig, hyperbolic, erf, and gamma implementations.
 *
...
#ifndef FDD_DETAIL_MATH_TRANSCENDENTAL_INCLUDED
#define FDD_DETAIL_MATH_TRANSCENDENTAL_INCLUDED
#include "fltx/detail/fdd_math_basic.h"
#include "fltx/fdd_numbers.h"

namespace bl {

[[nodiscard]] BL_FORCE_INLINE constexpr double detail::_dd_impl::log_as_double(fdd_s a);

namespace detail::_dd // primitives and kernels
{
    // polynomial evaluation helpers
    BL_FORCE_INLINE constexpr fdd_s horner_forward_inline(const fdd_s* coeffs, std::size_t count, const fdd_s& x) noexcept
    {
        if (count == 0)
            return {};
    ...
    BL_FORCE_INLINE constexpr fdd_s horner_forward(const fdd_s* coeffs, std::size_t count, const fdd_s& x) noexcept
    {
        ...
        return detail::_dd_runtime::horner_forward(coeffs, count, x);
    }

    BL_FORCE_INLINE constexpr fdd_s horner_reverse_inline(const fdd_s* coeffs, std::size_t count, const fdd_s& x) noexcept
    {
        if (count == 0)
            return {};
    ...
    BL_FORCE_INLINE constexpr fdd_s horner_reverse(const fdd_s* coeffs, std::size_t count, const fdd_s& x) noexcept
    {
        if (bl::detail::is_constant_evaluated())
        {
        ...
        return detail::_dd_runtime::horner_reverse(coeffs, count, x);
    }
        ...
        fdd_s& right_out) noexcept
    {
        ...
        detail::_dd_runtime::horner_pair_forward(left_coeffs, right_coeffs, count, x, left_out, right_out);
    }

    // expm1/log1p functions
    BL_MSVC_NOINLINE constexpr fdd_s log1p_series_reduced(const fdd_s& x)
    {
        const fdd_s z = div_prechecked_inline(x, add_double_finite_inline(x, 2.0));
        const fdd_s z2 = mul_product_inline(z, z);
        ...
        for (int k = 3; k <= 81; k += 2)
        {
            ...
            const fdd_s add = div_double_prechecked_inline(term, static_cast<double>(k));
            sum = add_finite_inline(sum, add);

            const fdd_s asum  = mag(sum);
            const fdd_s scale = (asum > fdd_s{ 1.0 }) ? asum : fdd_s{ 1.0 };
    ...
    BL_MSVC_NOINLINE constexpr fdd_s log_normalized_series(const fdd_s& m)
    {
        constexpr fdd_s inv_odd[] = {
        ...
        const fdd_s z = div_prechecked_inline(sub_double_finite_inline(m, 1.0), add_double_finite_inline(m, 1.0));
        const fdd_s z2 = mul_product_inline(z, z);

        fdd_s p = inv_odd[sizeof(inv_odd) / sizeof(inv_odd[0]) - 1];
    ...
    BL_MSVC_NOINLINE constexpr fdd_s expm1_tiny(const fdd_s& r)
    {
        constexpr int coeff_count = static_cast<int>(sizeof(exp_inv_fact) / sizeof(exp_inv_fact[0]));

        fdd_s p = mul_product_inline(r, r);
        fdd_s sum = add_finite_inline(r, mul_pwr2_inline(p, 0.5));
        const double threshold = absd(r.hi) * convergence_epsilon.hi;

        p = mul_product_inline(p, r);
        for (int i = 0; i < coeff_count; ++i)
        {
            const fdd_s term = mul_product_inline(p, exp_inv_fact[i]);
    ...
    BL_FORCE_INLINE constexpr fdd_s exp_integer_factor(
        int n,
        int& binary_scale) noexcept
    {
        ...
        const fdd_s* table = negative ? exp_integer_inv_table : exp_integer_table;
        fdd_s factor{ 1.0 };
    ...
    BL_FORCE_INLINE constexpr bool exp_overflows(const fdd_s& x) noexcept
    {
    ...
    BL_FORCE_INLINE constexpr bool exp_underflows_to_zero(const fdd_s& x) noexcept
    {
    ...
    BL_FORCE_INLINE constexpr int exp_reduction_integer(const fdd_s& x) noexcept
    {
        int n = static_cast<int>(exp_nearest_integer(x.hi));
        return n > 709 ? 709 : n;
    }

    BL_MSVC_NOINLINE constexpr fdd_s exp_general_scaled_with_n(const fdd_s& x, bool sub_one, int n) noexcept
    {
        const double nd = static_cast<double>(n);
        const fdd_s reduced = sub_double_finite_inline(x, nd);
        const fdd_s r = mul_pwr2_inline(reduced, 0.0078125);

        fdd_s e = expm1_tiny(r);
...
#else
        const fdd_s scaled = mul_add_inline(factor, e, factor);
#endif
        const fdd_s result = factor_scale == 0
            ? scaled
            : _ldexp(scaled, factor_scale);
        return sub_one ? sub_double_finite_inline(result, 1.0) : result;
    }

    BL_MSVC_NOINLINE constexpr fdd_s exp_general_scaled(const fdd_s& x, bool sub_one) noexcept
    {
        return exp_general_scaled_with_n(x, sub_one, exp_reduction_integer(x));
    }

    BL_MSVC_NOINLINE constexpr fdd_s _exp(const fdd_s& x)
    {
        if (isnan(x))
            return x;
        if (isinf(x))
            return (x.hi < 0.0) ? fdd_s{ 0.0 } : std::numeric_limits<fdd_s>::infinity();
    ...
    BL_MSVC_NOINLINE constexpr fdd_s _exp2(const fdd_s& x)
    {
        if (isnan(x))
            return x;
        if (isinf(x))
        ...
        if (x.hi > 1023.0 || x.hi < -1074.0)
            return _exp(mul_product_inline(x, std::numbers::ln2_v<fdd_s>));
        ...
        const fdd_s reduced = sub_double_finite_inline(x, kd);
        const fdd_s r = mul_pwr2_inline(mul_product_inline(reduced, std::numbers::ln2_v<fdd_s>), 0.0078125);
    ...
    // logarithm functions
    BL_MSVC_NOINLINE constexpr fdd_s _log(const fdd_s& a)
    {
        if (isnan(a))
            return a;
        if (iszero(a))
            return fdd_s{ -std::numeric_limits<double>::infinity(), 0.0 };
        ...
        const fdd_s exp2_ln2 = mul_double_product_inline(std::numbers::ln2_v<fdd_s>, static_cast<double>(exp2));
        return add_finite_inline(exp2_ln2, log_normalized_series(m));
    }

    BL_FORCE_INLINE constexpr bool dd_try_exact_binary_log2(const fdd_s& x, int& out) noexcept
    {
    ...
    // power functions
    [[nodiscard]] BL_FORCE_INLINE constexpr bool dd_try_pow10_ldexp_chunks(
        int pow5_count,
        ...
        fdd_s& out) noexcept
    {
        ...
        const int chunk_limit = exponent > 0
            ? detail::_dd::pow10_dd_max_exponent / pow5_count
            : (-detail::_dd::pow10_dd_min_exponent) / pow5_count;
        if (chunk_limit <= 0)
            return false;
        ...
        while (remaining != 0)
        {
            ...
            if (!detail::fp::checked_exponent_product(pow5_count, chunk, decimal_exponent))
                return false;
            if (decimal_exponent < detail::_dd::pow10_dd_min_exponent ||
                decimal_exponent > detail::_dd::pow10_dd_max_exponent)
            {
                return false;
            }
            ...
            const fdd_s term = detail::_dd_impl::ldexp(
                detail::_dd_impl::pow10_fdd(decimal_exponent),
                binary_exponent);
        ...
        fdd_s& out)
    {
        ...
        if (pow10_base)
        {
            value = detail::_dd_impl::pow10_fdd(exponent);
        }
        else
        {
            int binary_exponent = 0;
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr bool try_integral_pow_finite_raw(Base x, Exp y, fdd_s& out) noexcept
    {
            ...
            decimal_exponent <= detail::_dd::pow10_dd_max_exponent)
        {
            ...
            value = detail::_dd_impl::ldexp(
                detail::_dd_impl::pow10_fdd(decimal_exponent),
                binary_exponent);
        }
        else if (!dd_try_pow10_ldexp_chunks(
            pow5_count,
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s pow_mul_adaptive(const fdd_s& a, const fdd_s& b) noexcept
    {
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s pow_sqr_adaptive(const fdd_s& a) noexcept
    {
    ...
    template<class ExpUnsigned>
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s powi_nonnegative_fast(fdd_s base, ExpUnsigned exp) noexcept
    {
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s reciprocal_pow_result(const fdd_s& value) noexcept
    {
        if (detail::fp::iszero_or_inf_or_nan(value.hi)) [[unlikely]]
            return div_special(fdd_s{ 1.0 }, value);

        return div_double_prechecked_inline(1.0, value);
    }

    [[nodiscard]] BL_FORCE_INLINE constexpr bool try_exact_integer_pow_base_value(const fdd_s& value, std::int64_t& out) noexcept
    {
    ...
    template<detail::fp::non_bool_integral Exp>
    [[nodiscard]] BL_FORCE_INLINE constexpr bool try_exact_integer_pow_base(const fdd_s& x, Exp y, fdd_s& out) noexcept
    {
        std::int64_t integer_base = 0;
        if (!try_exact_integer_pow_base_value(x, integer_base))
            return false;

        if (detail::_dd::try_integral_pow_finite_raw(integer_base, y, out))
            return true;
        ...
        if constexpr (std::signed_integral<std::remove_cvref_t<Exp>>)
        {
            if (y < 0)
            {
                if (detail::fp::integral_pow_reciprocal_underflows_binary64(integer_base, magnitude))
                {
                    ...
                    out = detail::_dd::signed_zero(negative_zero);
                    return true;
                }

                if (split_safe)
                {
                    const fdd_s powered = detail::_dd::powi_nonnegative_unchecked(x, magnitude);
                    out = detail::_dd::reciprocal_pow_result(powered);
                    return true;
                }

                const fdd_s reciprocal = detail::_dd::reciprocal_pow_result(x);
                out = (integer_base != 0)
                    ? detail::_dd::powi_nonnegative_unchecked(reciprocal, magnitude)
                    : detail::_dd::powi_nonnegative_impl<false>(reciprocal, magnitude);
                return true;
            }
        }

        out = split_safe
            ? detail::_dd::powi_nonnegative_unchecked(x, magnitude)
            : detail::_dd::powi_nonnegative_fast<U>(x, magnitude);
        return true;
    }

    template<detail::fp::non_bool_integral Exp>
    [[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s ipow_integer(const fdd_s& x, Exp y)
    {
        using U = std::make_unsigned_t<std::remove_cvref_t<Exp>>;
        const U magnitude = detail::fp::unsigned_abs(y);
        ...
        if (isinf(x))
        {
            ...
            {
                if (y < 0)
                    return detail::_dd::signed_zero(negative);
            }
            return fdd_s{
                negative
        ...
        fdd_s special{};
        if (detail::_dd::try_exact_integer_pow_base(x, y, special))
            return special;
        ...
        if constexpr (std::signed_integral<std::remove_cvref_t<Exp>>)
        {
            ...
            {
                const fdd_s reciprocal = detail::_dd::reciprocal_pow_result(x);
                return detail::_dd::powi_nonnegative_impl<false>(reciprocal, magnitude);
            }
        }

        return detail::_dd::powi_nonnegative_fast<U>(x, magnitude);
    }

    BL_FORCE_INLINE constexpr fdd_s polish_eighth_root(const fdd_s& x, const fdd_s& y)
    {
        if (iszero(y))
            return y;

        const fdd_s y2 = mul_product_inline(y, y);
        const fdd_s y4 = mul_product_inline(y2, y2);
        const fdd_s y7 = mul_product_inline(mul_product_inline(y4, y2), y);
        const fdd_s y8 = mul_product_inline(y4, y4);
        const fdd_s correction = div_double_prechecked_inline(div_prechecked_inline(sub_finite_inline(x, y8), y7), 8.0);

        return add_finite_inline(y, correction);
    }

    BL_FORCE_INLINE constexpr fdd_s pow_positive_eighth_fraction(const fdd_s& x, int numerator)
    {
        const fdd_s r2 = detail::_dd_impl::sqrt(x);
        if (numerator == 4)
            return r2;

        const fdd_s r4 = detail::_dd_impl::sqrt(r2);
        if (numerator == 2)
            return r4;
        ...
        if ((numerator & 1) != 0)
        {
            const fdd_s r8 = polish_eighth_root(x, detail::_dd_impl::sqrt(r4));
            if (numerator == 1)
                return r8;
    ...
    BL_FORCE_INLINE constexpr bool try_get_pow_dyadic_eighth_exponent(const fdd_s& x, const fdd_s& y, int64_t& n)
    {
    ...
    BL_FORCE_INLINE constexpr bool try_get_pow_dyadic_eighth_exponent(const fdd_s& x, double y, int64_t& n) noexcept
    {
        if (x.hi < 0.0 || (x.hi == 0.0 && signbit(x.hi)))
            return false;
    ...
    BL_NO_INLINE constexpr fdd_s pow_dyadic_eighth_unchecked(const fdd_s& x, int64_t n)
    {
        if (n == 0)
            return fdd_s{ 1.0 };
        ...
        fdd_s result = (whole == 0u) ? fdd_s{ 1.0 } : detail::fp::powi_by_squaring(x, static_cast<int64_t>(whole));
        if (rem != 0)
    ...
    BL_FORCE_INLINE constexpr fdd_s pow_from_log_product(const fdd_s& product)
    {
        if (detail::_dd::mag(product) <= fdd_s{ 0.125 })
            return add_double_finite_inline(expm1_tiny(product), 1.0);

        return _exp(product);
    }

    BL_FORCE_INLINE constexpr fdd_s log_for_pow_positive(const fdd_s& x)
    {
        const fdd_s xm1 = sub_double_finite_inline(x, 1.0);
        if (detail::_dd::mag(xm1) <= fdd_s{ 0.5 })
            return log1p_series_reduced(xm1);

        return _log(x);
    }

    BL_FORCE_INLINE constexpr fdd_s pow_log_product_accurate(const fdd_s& y, const fdd_s& log_x)
    {
        return mul_accurate_inline(y, log_x);
    }

    // sine/cosine functions
    BL_FORCE_INLINE constexpr bool remainder_pio2(const fdd_s& x, long long& n_out, fdd_s& r_out)
    {
        ...
        const fdd_s t = mul_product_inline(x, invpi2);
        ...
        const fdd_s delta = sub_double_finite_inline(t, qd);
        ...
        constexpr fdd_s pi_2_hi{ pi_2_hi_d };
        constexpr fdd_s pi_2_mid{ pi_2_mid_d };
        constexpr fdd_s pi_2_lo{ pi_2_lo_d };
        constexpr fdd_s pi_4{ pi_4_hi };
    ...
    BL_MSVC_NOINLINE constexpr bool remainder_pio2_payne_hanek(const fdd_s& x, long long& n_out, fdd_s& r_out)
    {
        detail::exact_decimal::biguint magnitude;
        int binary_exp = 0;
        bool unused_neg = false;
        if (!detail::exact_decimal::exact_binary_components<dd_decimal_traits>(mag(x), magnitude, binary_exp, unused_neg))
        {
            n_out = 0;
            r_out = signbit(x) ? fdd_s{ -0.0, 0.0 } : fdd_s{ 0.0, 0.0 };
            return true;
        }
        ...
        fdd_s r = exact_dyadic_to_dd_fmod_big(y_coeff, -scale_bits, y_neg);
    ...
    BL_FORCE_INLINE constexpr void sincos_kernel_small(const fdd_s& x, fdd_s& s_out, fdd_s& c_out)
    {
        ...
        const fdd_s xt = mul_product_inline(x, t);
        s_out = mul_add_inline(xt, ps, x);
        c_out = mul_add_double_rhs_inline(t, pc, 1.0);
    }

    BL_FORCE_INLINE constexpr void sincos_kernel_pi64_reduced(const fdd_s& x, fdd_s& s_out, fdd_s& c_out)
    {
        ...
        const fdd_s a = mul_double_product_inline(std::numbers::pi_v<fdd_s>, static_cast<double>(k) * 0.015625);
        const fdd_s u = sub_finite_inline(x, a);

        fdd_s su{}, cu{};
        sincos_kernel_small(u, su, cu);

        const int table_index = k < 0 ? -k : k;
        const fdd_s sa = k < 0 ? -dd_sin_table_pi64[table_index] : dd_sin_table_pi64[table_index];
        const fdd_s ca = dd_cos_table_pi64[table_index];

        s_out = sum_products_inline(ca, su, sa, cu);
        c_out = diff_products_inline(ca, cu, sa, su);
    }

    BL_FORCE_INLINE constexpr fdd_s sin_kernel_pi4(const fdd_s& x)
    {
        if (detail::fp::absd(x.hi) < 0x1p-56)
            return x;
        ...
        const fdd_s ps = horner_forward(dd_sin_coeffs_pi4, dd_trig_coeff_count_pi4, t);

        return mul_add_inline(mul_product_inline(x, t), ps, x);
    }

    BL_FORCE_INLINE constexpr fdd_s cos_kernel_pi4(const fdd_s& x)
    {
        const fdd_s t = mul_product_inline(x, x);

        const fdd_s pc = horner_forward(dd_cos_coeffs_pi4, dd_trig_coeff_count_pi4, t);

        return mul_add_double_rhs_inline(t, pc, 1.0);
    }

    // inverse trig functions
    BL_FORCE_INLINE constexpr fdd_s atan_series_reduced(const fdd_s& z)
    {
        constexpr int count = static_cast<int>(sizeof(dd_atan_reduced_coeffs) / sizeof(dd_atan_reduced_coeffs[0]));

        const fdd_s z2 = mul_product_inline(z, z);
        const fdd_s p = horner_reverse(dd_atan_reduced_coeffs, static_cast<std::size_t>(count), z2);

        return mul_product_inline(z, p);
    }

    BL_FORCE_INLINE constexpr fdd_s atan_core_unit(const fdd_s& z)
    {
        ...
        const fdd_s u = div_prechecked_inline(
            sub_double_finite_inline(z, a),
            add_double_finite_inline(mul_double_product_inline(z, a), 1.0));

        return add_finite_inline(dd_atan_reduced_table_16[k], atan_series_reduced(u));
    }

    BL_FORCE_INLINE constexpr fdd_s _atan(const fdd_s& x)
    {
        if (isnan(x))  return x;
        ...
        const fdd_s ax = neg ? -x : x;

        if (ax > fdd_s{ 1.0 })
        {
            const fdd_s core = atan_core_unit(div_double_prechecked_inline(1.0, ax));
            const fdd_s out  = sub_finite_inline(pi_2, core);
            return neg ? -out : out;
        }

        const fdd_s out = atan_core_unit(ax);
        return neg ? -out : out;
    }

    BL_FORCE_INLINE constexpr fdd_s _asin(const fdd_s& x)
    {
        if (isnan(x))
            return x;
        ...
        if (ax <= fdd_s{ 0.5 })
            return _atan(div_prechecked_inline(x, detail::_dd_impl::sqrt(sub_double_finite_inline(1.0, mul_product_inline(x, x)))));

        const fdd_s t = detail::_dd_impl::sqrt(div_prechecked_inline(sub_double_finite_inline(1.0, ax), add_double_finite_inline(ax, 1.0)));
        const fdd_s a = sub_finite_inline(pi_2, mul_double_product_inline(_atan(t), 2.0));
        return (x.hi < 0.0) ? -a : a;
    }

    BL_FORCE_INLINE constexpr fdd_s _acos(const fdd_s& x)
    {
        if (isnan(x))
            return x;
        ...
        if (x == fdd_s{ 1.0 })
            return fdd_s{ 0.0 };
        if (x == fdd_s{ -1.0 })
            return std::numbers::pi_v<fdd_s>;

        return sub_finite_inline(pi_2, detail::_dd::_asin(x));
    }

    // inverse hyperbolic functions
    BL_MSVC_NOINLINE constexpr fdd_s atanh_small_series(const fdd_s& x)
    {
        const fdd_s x2 = mul_product_inline(x, x);
        fdd_s sum   = x;
        fdd_s power = x;
        ...
        for (int k = 1; k <= 32; ++k)
        {
            ...
            const fdd_s term = div_double_prechecked_inline(power, static_cast<double>(2 * k + 1));
            sum = add_finite_inline(sum, term);
    ...
    // erf/erfc functions
    [[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s erf_cheb_eval(
        const fdd_s& x,
        const fdd_s* coeffs,
        std::size_t count,
        double shift)
    {
        const fdd_s t = sub_double_finite_inline(mul_double_product_inline(x, 2.0), shift);
        fdd_s b1{ 0.0 };
        fdd_s b2{ 0.0 };

        for (int i = static_cast<int>(count) - 1; i >= 1; --i)
        {
            const fdd_s b0 = add_finite_inline(
    ...
    [[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s erf_positive_cheb_1_2(const fdd_s& x)
    {
        return erf_cheb_eval(x, dd_erf_cheb_1_2, dd_erf_cheb_1_2_coeff_count, 3.0);
    }

    BL_FORCE_INLINE constexpr fdd_s erf_positive_series(const fdd_s& x)
    {
        const fdd_s xx = mul_product_inline(x, x);
        fdd_s power = x;
        fdd_s sum   = x;
        ...
        for (int n = 1; n < 256; ++n)
        {
            ...
            const fdd_s term = div_double_prechecked_inline(power, static_cast<double>(2 * n + 1));
            sum = add_finite_inline(sum, term);
            if (mag(term) < convergence_epsilon)
                break;
        }

        return mul_pwr2_inline(mul_product_inline(std::numbers::inv_sqrtpi_v<fdd_s>, sum), 2.0);
    }

    BL_FORCE_INLINE constexpr fdd_s erfc_positive_cf(const fdd_s& x)
    {
        const fdd_s z = mul_product_inline(x, x);
        constexpr fdd_s tiny = fdd_s{ 1.0e-300 };
        constexpr fdd_s convergence = fdd_s{ 0x1p-101 }; // 32 * convergence_epsilon

        fdd_s b = add_double_finite_inline(z, 0.5);
        fdd_s c = div_double_prechecked_inline(1.0, tiny);
        fdd_s d = div_double_prechecked_inline(1.0, b);
        fdd_s h = d;
        ...
        for (int i = 1; i <= 96; ++i)
        {
            ...
            const fdd_s delta = mul_product_inline(d, c);
    ...
    // gamma functions
    BL_MSVC_NOINLINE constexpr fdd_s lgamma1p_series(const fdd_s& y) noexcept
    {
        constexpr int count = static_cast<int>(sizeof(lgamma1p_coeff) / sizeof(lgamma1p_coeff[0]));

        const fdd_s p = horner_reverse(lgamma1p_coeff, static_cast<std::size_t>(count), y);

        return mul_product_inline(y, mul_add_inline(y, p, -std::numbers::egamma_v<fdd_s>));
    }

    BL_MSVC_NOINLINE constexpr fdd_s lgamma1p5_series(const fdd_s& y) noexcept
    {
        constexpr int count = static_cast<int>(sizeof(lgamma1p5_coeff) / sizeof(lgamma1p5_coeff[0]));

        const fdd_s p = horner_reverse(lgamma1p5_coeff, static_cast<std::size_t>(count), y);

        const fdd_s constant = sub_finite_inline(half_log_two_pi, mul_double_product_inline(std::numbers::ln2_v<fdd_s>, 1.5));
        const fdd_s linear   = sub_finite_inline(sub_double_finite_inline(2.0, std::numbers::egamma_v<fdd_s>), mul_double_product_inline(std::numbers::ln2_v<fdd_s>, 2.0));
        return mul_add_inline(y, mul_add_inline(y, p, linear), constant);
    }

    BL_MSVC_NOINLINE constexpr bool try_lgamma_near_one_or_two(const fdd_s& x, fdd_s& out) noexcept
    {
        const fdd_s y1 = sub_double_finite_inline(x, 1.0);
        if (mag(y1) <= fdd_s{ 0.25 })
        {
            out = lgamma1p_series(y1);
            return true;
        }

        const fdd_s y15 = sub_double_finite_inline(x, 1.5);
        if (mag(y15) <= fdd_s{ 0.25 })
        {
            out = lgamma1p5_series(y15);
            return true;
        }

        const fdd_s y2 = sub_double_finite_inline(x, 2.0);
        if (mag(y2) <= fdd_s{ 0.25 })
        {
    ...
    BL_MSVC_NOINLINE constexpr fdd_s lgamma_stirling_asymptotic(const fdd_s& z) noexcept
    {
        const fdd_s inv    = div_double_prechecked_inline(1.0, z);
        const fdd_s inv2   = mul_product_inline(inv, inv);
        const fdd_s series = mul_product_inline(inv, horner_reverse(
            lgamma_stirling_coeffs,
            sizeof(lgamma_stirling_coeffs) / sizeof(lgamma_stirling_coeffs[0]),
            inv2));

        return add_finite_inline(add_finite_inline(sub_finite_inline(mul_product_inline(sub_double_finite_inline(z, 0.5), detail::_dd_impl::log(z)), z), half_log_two_pi), series);
    }

    BL_MSVC_NOINLINE constexpr void positive_recurrence_product(const fdd_s& x, const fdd_s& asymptotic_min, fdd_s& z, fdd_s& product, int& product_scale2) noexcept
    {
        z = x;
        product = fdd_s{ 1.0 };
        product_scale2 = 0;
        ...
        while (z < asymptotic_min)
        {
            ...
            if (hi != 0.0)
            {
                ...
                {
                    product = detail::_dd_impl::ldexp(product, -exponent);
    ...
    BL_MSVC_NOINLINE constexpr fdd_s lgamma_positive_low_range(const fdd_s& x) noexcept
    {
        fdd_s y = x;
        fdd_s product{ 1.0 };
        ...
        if (product == fdd_s{ 1.0 })
            return local;

        const fdd_s correction = detail::_dd_impl::log(product);
        return shifted_up ? sub_finite_inline(local, correction) : add_finite_inline(local, correction);
    }

    BL_MSVC_NOINLINE constexpr fdd_s gamma_positive_low_range(const fdd_s& x) noexcept
    {
        fdd_s y = x;
        fdd_s product{ 1.0 };
        bool shifted_up = false;
    ...
    BL_MSVC_NOINLINE constexpr fdd_s lgamma_positive_recurrence(const fdd_s& x) noexcept
    {
        fdd_s near_value{};
        ...
        constexpr fdd_s asymptotic_min = fdd_s{ 40.0 };
        ...
        return sub_finite_inline(
            sub_finite_inline(lgamma_stirling_asymptotic(z), detail::_dd_impl::log(product)),
            mul_double_product_inline(std::numbers::ln2_v<fdd_s>, static_cast<double>(product_scale2)));
    }

    BL_MSVC_NOINLINE constexpr fdd_s gamma_positive_recurrence(const fdd_s& x) noexcept
    {
        fdd_s near_lgamma{};
        if (try_lgamma_near_one_or_two(x, near_lgamma))
            return detail::_dd_impl::exp(near_lgamma);
...
#else
        fdd_s out = div_prechecked_inline(detail::_dd_impl::exp(lgamma_stirling_asymptotic(z)), product);
#endif
        if (product_scale2 != 0)
            out = detail::_dd_impl::ldexp(out, -product_scale2);

        return out;
    }

    BL_MSVC_NOINLINE constexpr fdd_s sinpi_reduced(const fdd_s& x) noexcept
    {
        const fdd_s n = detail::_dd_impl::round_nearest_even(x);
        const fdd_s r = sub_finite_inline(x, n);
        fdd_s out = detail::_dd_impl::sin(mul_product_inline(std::numbers::pi_v<fdd_s>, r));
        if (is_odd_integer(n))
            out = -out;
        return out;
    }

    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s cbrt_constexpr_seed(const fdd_s& ax)
    {
        return fdd_s{ detail::fp::cbrt_seed(ax.hi), 0.0 };
    }
}

namespace detail::_dd_runtime
{
    [[nodiscard]] BL_FORCE_INLINE fdd_s cbrt_seed(const fdd_s& ax)
    {
        int exp2 = 0;
        ...
        fdd_s y{ std::cbrt(mantissa), 0.0 };
        if (exp2 != 0)
            y = detail::_dd::_ldexp(y, exp2 / 3);
        return y;
    }
}
...
{
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s cbrt_seed(const fdd_s& ax)
    {
        BL_CONSTEXPR_RUNTIME_DISPATCH(
            cbrt_constexpr_seed(ax),
            detail::_dd_runtime::cbrt_seed(ax)
        );
    }

    BL_PUSH_PRECISE;
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s cbrt_compensated(const fdd_s& ax, double c) noexcept
    {
        double c2_hi{}, c2_lo{};
        ...
        const fdd_s residual = renorm(residual_hi, residual_lo);
...
// exponential functions
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::exp(const fdd_s& x)
{
    return _exp(x);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::exp2(const fdd_s& x)
{
    return _exp2(x);
}
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::log(const fdd_s& a)
{
    return _log(a);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::log2(const fdd_s& a)
{
    if (isnan(a))
        return a;
    if (iszero(a))
    ...
    int exact_exp2{};
    if (dd_try_exact_binary_log2(a, exact_exp2))
        return fdd_s{ static_cast<double>(exact_exp2), 0.0 };

    return mul_product_inline(_log(a), std::numbers::log2e_v<fdd_s>);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::log10(const fdd_s& x)
{
    if (isnan(x))
        return x;
    if (iszero(x))
    ...
    if (detail::fp::isfinite(x.hi) && x.hi > 0.0 && x.lo == 0.0)
    {
        ...
        {
            const fdd_s pow10 = detail::_dd_impl::pow10_fdd(k);
            if (pow10.hi >= std::numeric_limits<double>::min() && x == pow10)
                return fdd_s{ static_cast<double>(k), 0.0 };
        }
    }

    return mul_product_inline(_log(x), std::numbers::log10e_v<fdd_s>);
}

// expm1/log1p functions
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::expm1(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    if (isinf(x))
        return signbit(x)
            ? fdd_s{ -1.0, 0.0 }
            : std::numeric_limits<fdd_s>::infinity();

    if (exp_overflows(x))
        return std::numeric_limits<fdd_s>::infinity();

    if (exp_underflows_to_zero(x))
        return fdd_s{ -1.0, 0.0 };

    return exp_general_scaled(x, true);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::log1p(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    const fdd_s u = add_double_finite_inline(x, 1.0);
    return _log(u);
}

// roots
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::cbrt(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    fdd_s y = cbrt_compensated(ax, cbrt_seed(ax).hi);
    if (bl::detail::is_constant_evaluated())
...
// power functions
[[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s detail::_dd_impl::pow(const fdd_s& x, const fdd_s& y)
{
    if (iszero(y))
        return fdd_s{ 1.0 };

    if (x == fdd_s{ 1.0 } || (x == fdd_s{ -1.0 } && detail::fp::isinf(y.hi)))
        return fdd_s{ 1.0 };
    ...
    if (detail::fp::isinf(y.hi))
    {
        ...
        if (ax == fdd_s{ 1.0 })
            return fdd_s{ 1.0 };
        if (ax < fdd_s{ 1.0 })
            return signbit(y) ? std::numeric_limits<fdd_s>::infinity() : fdd_s{ 0.0 };
        return signbit(y) ? fdd_s{ 0.0 } : std::numeric_limits<fdd_s>::infinity();
    }

    const fdd_s yi = detail::_dd_impl::trunc(y);
    ...
    if (x.hi < 0.0 || (x.hi == 0.0 && signbit(x.hi)))
    {
        ...
        const fdd_s magnitude = pow_from_log_product(pow_log_product_accurate(y, log_for_pow_positive(-x)));
...
[[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s detail::_dd_impl::pow(const fdd_s& x, double y)
{
    if (y == 0.0)
        return fdd_s{ 1.0 };

    if (x == fdd_s{ 1.0 } || (x == fdd_s{ -1.0 } && detail::fp::isinf(y)))
        return fdd_s{ 1.0 };
    ...
    if (detail::fp::isinf(y))
    {
        ...
        if (ax == fdd_s{ 1.0 })
            return fdd_s{ 1.0 };
        if (ax < fdd_s{ 1.0 })
            return detail::fp::signbit(y) ? std::numeric_limits<fdd_s>::infinity() : fdd_s{ 0.0 };
        return detail::fp::signbit(y) ? fdd_s{ 0.0 } : std::numeric_limits<fdd_s>::infinity();
    }

    if (y == 1.0)  return x;
    if (y == 2.0)  return x * x;
    if (y == -1.0) return div_double_prechecked_inline(1.0, x);
    if (y == 0.5)  return detail::_dd_impl::sqrt(x);
    ...
    if (x.hi < 0.0 || (x.hi == 0.0 && signbit(x.hi)))
    {
        ...
        const fdd_s magnitude = pow_from_log_product(log_for_pow_positive(-x) * y);
        const bool y_is_odd =
            (absd(yi) < 0x1p53) &&
...
// inverse trig functions
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::atan(const fdd_s& x)
{
    return detail::_dd::_atan(x);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::asin(const fdd_s& x)
{
    return detail::_dd::_asin(x);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::acos(const fdd_s& x)
{
    return detail::_dd::_acos(x);
}

// sine/cosine functions
[[nodiscard]] BL_MSVC_NOINLINE constexpr bool detail::_dd_impl::sincos(const fdd_s& x, fdd_s& s_out, fdd_s& c_out)
{
    if (iszero(x))
    {
        s_out = x;
    ...
    const double ax = detail::_dd::fabs(x.hi);
    if (detail::fp::isinf_or_nan(ax))
    {
        s_out = fdd_s{ std::numeric_limits<double>::quiet_NaN() };
        c_out = s_out;
        return false;
    }
...
[[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s detail::_dd_impl::sin(const fdd_s& x)
{
    if (iszero(x))
        return x;
    ...
    const double ax = detail::_dd::fabs(x.hi);
    if (detail::fp::isinf_or_nan(ax))
        return fdd_s{ std::numeric_limits<double>::quiet_NaN() };
...
[[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s detail::_dd_impl::cos(const fdd_s& x)
{
    const double ax = detail::_dd::fabs(x.hi);
    if (detail::fp::isinf_or_nan(ax))
        return fdd_s{ std::numeric_limits<double>::quiet_NaN() };
...
// tangent and atan2
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::tan(const fdd_s& x)
{
    if (iszero(x))
        return x;

    fdd_s s{}, c{};
    if (detail::_dd_impl::sincos(x, s, c))
        return div_prechecked_inline(s, c);
    const double xd = (double)x;
    if (bl::detail::is_constant_evaluated()) {
        return fdd_s{ detail::fp::tan(xd) };
    } else {
        return fdd_s{ std::tan(xd) };
    }
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::atan2(const fdd_s& y, const fdd_s& x)
{
    if (detail::fp::isnan(x.hi) || detail::fp::isnan(y.hi))
        return std::numeric_limits<fdd_s>::quiet_NaN();
    ...
    if (isinf(y))
    {
        ...
        {
            if (x.hi < 0.0)
                return signbit(y) ? -detail::_dd::pi_3_4 : detail::_dd::pi_3_4;
            return signbit(y) ? -detail::_dd::pi_4 : detail::_dd::pi_4;
        }

        return signbit(y) ? -detail::_dd::pi_2 : detail::_dd::pi_2;
    }
    if (isinf(x))
    {
        if (x.hi < 0.0)
            return signbit(y) ? -std::numbers::pi_v<fdd_s> : std::numbers::pi_v<fdd_s>;
        return detail::_dd::signed_zero(signbit(y));
    }
    ...
    if (iszero(x))
    {
        ...
        return ispositive(y) ? detail::_dd::pi_2 : -detail::_dd::pi_2;
    }

    if (iszero(y))
    {
        if (x.hi < 0.0)
            return signbit(y.hi) ? -std::numbers::pi_v<fdd_s> : std::numbers::pi_v<fdd_s>;
        return y;
    }

    const fdd_s ax = detail::_dd::mag(x);
    const fdd_s ay = detail::_dd::mag(y);
    ...
    if (ax == ay)
    {
        ...
        {
            return
                (y.hi < 0.0) ? -detail::_dd::pi_3_4 : detail::_dd::pi_3_4;
        }

        return
            (y.hi < 0.0) ? -detail::_dd::pi_4 : detail::_dd::pi_4;
    }
    ...
    if (ax >= ay)
    {
        ...
        #else
        const fdd_s ratio = detail::_dd::div_prechecked_inline(y, x);
        #endif
        fdd_s a = detail::_dd::_atan(ratio);

        if (x.hi < 0.0)
            a = detail::_dd::add_finite_inline(a, (y.hi < 0.0) ? -std::numbers::pi_v<fdd_s> : std::numbers::pi_v<fdd_s>);
        return a;
    }
    ...
    #else
    const fdd_s ratio = detail::_dd::div_prechecked_inline(x, y);
    #endif
    const fdd_s a = detail::_dd::_atan(ratio);
    return
        (y.hi < 0.0) ? detail::_dd::sub_finite_inline(-detail::_dd::pi_2, a) : detail::_dd::sub_finite_inline(detail::_dd::pi_2, a);
}

// sinh/cosh/tanh
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::sinh(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    if (ax <= fdd_s{ 0.5 })
    {
        const fdd_s e = detail::_dd_impl::expm1(x);
        return
...
#else
    fdd_s out = mul_double_product_inline(sub_finite_inline(ex, div_double_prechecked_inline(1.0, ex)), 0.5);
#endif
    if (signbit(x))
        out = -out;
    return out;
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::cosh(const fdd_s& x)
{
    using namespace detail::_dd;
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::tanh(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    if (detail::fp::iszero_or_nan(x.hi))
        return x;
    if (isinf(x))
        return signbit(x) ? fdd_s{ -1.0 } : fdd_s{ 1.0 };
    ...
    if (ax >= fdd_s{ 0.5 })
    {
        const fdd_s e = detail::_dd_impl::exp(mul_double_product_inline(ax, -2.0));
        fdd_s out = sub_double_finite_inline(1.0, div_prechecked_inline(mul_double_product_inline(e, 2.0), add_double_finite_inline(e, 1.0)));
        if (signbit(x))
            out = -out;
        return out;
    }
    ...
    const fdd_s e = detail::_dd_impl::expm1(x);
    const fdd_s ep1 = add_double_finite_inline(e, 1.0);
    return
...
// inverse hyperbolic functions
[[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s detail::_dd_impl::asinh(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    fdd_s out{};
    if (ax > fdd_s{ 0x1p500 })
        out = add_finite_inline(detail::_dd_impl::log(ax), std::numbers::ln2_v<fdd_s>);
    else if (ax <= fdd_s{ 0.5 })
    {
        const fdd_s ax2 = mul_product_inline(ax, ax);
        out = detail::_dd_impl::log1p(add_finite_inline(
            ax,
            div_prechecked_inline(ax2, add_double_finite_inline(detail::_dd_impl::sqrt(add_double_finite_inline(ax2, 1.0)), 1.0))));
    }
    else
        out = detail::_dd_impl::log(add_finite_inline(ax, detail::_dd_impl::sqrt(add_double_finite_inline(mul_product_inline(ax, ax), 1.0))));
...
[[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s detail::_dd_impl::acosh(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    fdd_s out{};
    if (x > fdd_s{ 0x1p500 })
        out = add_finite_inline(detail::_dd_impl::log(x), std::numbers::ln2_v<fdd_s>);
    else if (x < fdd_s{ 1.25 })
    {
        const fdd_s xm1 = sub_double_finite_inline(x, 1.0);
        out = detail::_dd_impl::log1p(add_finite_inline(
            xm1,
            detail::_dd_impl::sqrt(mul_product_inline(xm1, add_double_finite_inline(x, 1.0)))));
    }
    else
        out = detail::_dd_impl::log(add_finite_inline(
            x,
            detail::_dd_impl::sqrt(mul_product_inline(sub_double_finite_inline(x, 1.0), add_double_finite_inline(x, 1.0)))));

    return out;
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::atanh(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    if (ax == fdd_s{ 1.0 })
        return signbit(x)
            ? fdd_s{ -std::numeric_limits<double>::infinity(), 0.0 }
            : fdd_s{  std::numeric_limits<double>::infinity(), 0.0 };
    ...
    if (ax < fdd_s{ 0.25 })
    {
        const fdd_s r = div_prechecked_inline(mul_double_product_inline(x, 2.0), sub_double_finite_inline(1.0, x));
        return mul_double_product_inline(detail::_dd_impl::log1p(r), 0.5);
    }

    const fdd_s out = mul_double_product_inline(
        detail::_dd_impl::log(div_prechecked_inline(add_double_finite_inline(x, 1.0), sub_double_finite_inline(1.0, x))),
        0.5);
    return out;
}

// erf/erfc functions
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::erf(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    else if (ax < fdd_s{ 2.0 })
        out = erf_positive_cheb_1_2(ax);
    else
        out = ax > fdd_s{ 27.0 } ? fdd_s{ 1.0 } : sub_double_finite_inline(1.0, erfc_positive_cf(ax));
...
[[nodiscard]] BL_MSVC_NOINLINE constexpr fdd_s detail::_dd_impl::erfc(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    if (x == fdd_s{ 0.0 })
        return fdd_s{ 1.0 };
    if (isinf(x))
        return signbit(x) ? fdd_s{ 2.0 } : fdd_s{ 0.0 };
...
// gamma functions
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::lgamma(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    const fdd_s out =
        sub_finite_inline(
            sub_finite_inline(detail::_dd_impl::log(std::numbers::pi_v<fdd_s>), detail::_dd_impl::log(detail::_dd::mag(sinpix))),
            lgamma_positive_recurrence(sub_double_finite_inline(1.0, x)));

    return out;
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::tgamma(const fdd_s& x)
{
    using namespace detail::_dd;
    ...
    if (isinf(x))
        return signbit(x)
            ? std::numeric_limits<fdd_s>::quiet_NaN()
            : std::numeric_limits<fdd_s>::infinity();
    if (iszero(x))
        return fdd_s{ signbit(x) ? -std::numeric_limits<double>::infinity() : std::numeric_limits<double>::infinity(), 0.0 };
...
#endif // DD_DETAIL_MATH_TRANSCENDENTAL_INCLUDED
