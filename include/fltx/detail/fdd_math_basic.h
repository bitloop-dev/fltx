/**
 * fltx/detail/fdd_math_basic.h - Basic math implementation details.
 *
 * dd rounding, decomposition, remainder, hypot, and adjacent-value implementations.
 *
...
#ifndef FDD_DETAIL_MATH_BASIC_INCLUDED
#define FDD_DETAIL_MATH_BASIC_INCLUDED
#include "fltx/detail/fdd_math_kernels.h"
...
{
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s hypot_sqrt_sum(const fdd_s& sum)
    {
        BL_CONSTEXPR_RUNTIME_DISPATCH(
            detail::_dd_impl::sqrt(sum),
            detail::_dd::sqrt_compensated(sum, std::sqrt(sum.hi))
        );
    }
}
...
namespace detail::_dd
{
    inline constexpr int pow10_dd_min_exponent = detail::pow_tables::pow10_min_exponent;
    inline constexpr int pow10_dd_max_exponent = detail::pow_tables::pow10_max_exponent;

    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s pow_table_entry_to_dd(const detail::pow_table_entry& row) noexcept
    {
        return fdd_s{ row.x0, row.x1 };
    }
    ...
    BL_PUSH_PRECISE;
    [[nodiscard]] BL_NO_INLINE constexpr fdd_s fma_cancellation_exact(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z) noexcept
    {
        double expansion[10]{};
        int count = 0;
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s fma_finite_from_products(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z,
        fma_products products) noexcept
    {
        ...
        const fdd_s rough = renorm(hi, lo + tail);

        return rough;
    }

    [[nodiscard]] BL_FORCE_INLINE fdd_s fma_finite_hardware(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z) noexcept
    {
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s fma_finite_dekker(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z) noexcept
    {
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s fma_finite_auto(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z) noexcept
    {
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s fma_finite_uncancelled(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z) noexcept
    {
    ...
    [[nodiscard]] BL_NO_INLINE constexpr fdd_s fma_zero_leading_product(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z) noexcept
    {
        if ((value_is_zero(x) || value_is_zero(y)) && value_is_zero(z))
        {
            const bool product_negative = bl::signbit(x) != bl::signbit(y);
            return signed_zero(product_negative && bl::signbit(z));
        }

        return fma_finite_uncancelled(x, y, z);
    }

    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s fma_finite_dispatch(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z,
        double leading_product) noexcept
    {
        if (leading_product == 0.0) [[unlikely]]
            return fma_zero_leading_product(x, y, z);
        else if (detail::fp::absd(leading_product + z.hi)
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s fma_overflow_scaled(
        const fdd_s& x,
        const fdd_s& y,
        const fdd_s& z) noexcept
    {
        ...
        const fdd_s scaled_x = _ldexp(x, -half_scale);
        const fdd_s scaled_y = _ldexp(y, -half_scale);
        const fdd_s scaled_z = _ldexp(z, -2 * half_scale);
        const double scaled_leading_product = scaled_x.hi * scaled_y.hi;
        const fdd_s scaled_result =
            fma_finite_dispatch(scaled_x, scaled_y, scaled_z, scaled_leading_product);
        const fdd_s result = _ldexp(scaled_result, 2 * half_scale);

        if (detail::fp::isinf(result.hi)) [[unlikely]]
            return fdd_s{ result.hi, 0.0 };

        return result;
    }

    [[nodiscard]] BL_NO_INLINE constexpr fdd_s round_nearest_even_large(const fdd_s& a)
    {
        fdd_s t = detail::_dd_impl::floor(a);
        fdd_s frac = sub_finite_inline(a, t);
        ...
        if (detail::_dd_impl::fmod(t, fdd_s{ 2.0 }) != fdd_s{ 0.0 })
            t = add_double_finite_inline(t, 1.0);
    ...
    [[nodiscard]] BL_FORCE_INLINE constexpr fdd_s round_nearest_even(const fdd_s& a)
    {
        if (detail::fp::iszero_or_inf_or_nan(a.hi))
            return a;

        if (detail::_dd::absd(a.hi) < 0x1p52)
        {
            ...
            const double abs_frac_lo = detail::_dd::absd(a.lo);
            ...
            {
                const double abs_frac_hi = detail::_dd::absd(frac_hi);
                if (abs_frac_hi > 0.5 + abs_frac_lo)
                {
                    rounded += (frac_hi < 0.0 ||
                                (frac_hi == 0.0 && detail::_dd::signbit(a.lo))) ? -1 : 1;
                }
                else if (abs_frac_hi >= 0.5 - abs_frac_lo)
                {
                    const fdd_s frac = sub_double_finite_inline(a, base_d);
                    const fdd_s abs_frac = detail::_dd::mag(frac);
                    if (abs_frac > fdd_s{ 0.5 } ||
                        (abs_frac == fdd_s{ 0.5 } && (base & 1ll) != 0))
                    {
...
{
    [[nodiscard]] inline BL_NO_INLINE fdd_s round_nearest_away_from_zero_integral_head(
        const fdd_s& a) noexcept
    {
        return detail::_dd::round_nearest_away_from_zero(a);
    }
...
// roots
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::sqrt(fdd_s a)
{
    ...
    if (bl::detail::is_constant_evaluated() || a.hi < fast_min || a.hi > fast_max)
    {
        ...
        const fdd_s scaled_a = input_scale == 0 ? a : ldexp_terms(a, input_scale);

        double seed{};
        if (bl::detail::is_constant_evaluated())
            seed = detail::_dd::sqrt_constexpr_head(scaled_a.hi);
        else
            seed = std::sqrt(scaled_a.hi);

        fdd_s y = detail::_dd::sqrt_compensated(scaled_a, seed);
    ...
    return detail::_dd::sqrt_compensated(a, std::sqrt(a.hi));
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::hypot(const fdd_s& x, const fdd_s& y)
{
    ...
    fdd_s ax = detail::_dd::mag(x);
    fdd_s ay = detail::_dd::mag(y);
    if (ax < ay)
    {
        const fdd_s tmp = ax;
        ax = ay;
    ...
    {
        const fdd_s sum = add_finite_inline(sqr_inline(ax), sqr_inline(ay));
        return hypot_sqrt_sum(sum);
    }

    const fdd_s r = div_prechecked_inline(ay, ax);
    return mul_product_inline(ax, detail::_dd_impl::sqrt(add_double_finite_inline(mul_product_inline(r, r), 1.0)));
}

// rounding and decimals
[[nodiscard]] BL_FORCE_INLINE constexpr double detail::_dd_impl::floor_limb(double x) noexcept
{
    BL_CONSTEXPR_RUNTIME_DISPATCH(
        detail::fp::floor(x),
        std::floor(x)
...
[[nodiscard]] BL_FORCE_INLINE constexpr double detail::_dd_impl::ceil_limb(double x) noexcept
{
    BL_CONSTEXPR_RUNTIME_DISPATCH(
        detail::fp::ceil(x),
        std::ceil(x)
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::floor(const fdd_s& a)
{
    double hi = detail::_dd_impl::floor_limb(a.hi);
    double lo = 0.0;
    ...
    if (hi == a.hi)
    {
        lo = detail::_dd_impl::floor_limb(a.lo);
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::ceil(const fdd_s& a)
{
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::trunc(const fdd_s& a)
{
    ...
    if (detail::_dd::absd(a.hi) < detail::fp::double_integer_threshold)
    {
        ...
        if (hi == 0.0)
            return detail::_dd::signed_zero(signbit(a));
        return fdd_s{ hi, 0.0 };
    }

    return (a.hi < 0.0) ? detail::_dd_impl::ceil(a) : detail::_dd_impl::floor(a);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::round_nearest_away_from_zero(
    const fdd_s& a)
{
    BL_CONSTEXPR_RUNTIME_DISPATCH(
        detail::_dd::round_nearest_away_from_zero(a),
        detail::_dd_impl::round_nearest_away_from_zero_runtime(a)
    );
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::round_decimals(fdd_s v, int prec)
{
    ...
    bool neg = false;
    if (!detail::exact_decimal::exact_decimal_places_integer<detail::_dd::dd_significant_decimal_traits>(
            v,
            prec,
            coefficient,
            neg))
    ...
    const fdd_s rounded =
        detail::_dd::round_decimal_exact_to_dd(coefficient, -prec, neg);
    if (iszero(rounded))
        return fdd_s{ detail::fp::copysign(0.0, v.hi), 0.0 };
    return rounded;
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::round_significant(fdd_s v, int figures)
{
    ...
    const fdd_s ax = neg ? -v : v;

    detail::exact_decimal::biguint coefficient;
    int exp10 = 0;
    if (!detail::exact_decimal::exact_significant_decimal<detail::_dd::dd_significant_decimal_traits>(
            ax,
            figures,
            coefficient,
            exp10))
    ...
    return detail::_dd::round_decimal_exact_to_dd(coefficient, exp10 - (figures - 1), neg);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::pow10_fdd(int k)
{
    if (k < detail::_dd::pow10_dd_min_exponent) [[unlikely]]
        return fdd_s{ 0.0 };
    if (k > detail::_dd::pow10_dd_max_exponent) [[unlikely]]
        return std::numeric_limits<fdd_s>::infinity();

    return detail::_dd::pow_table_entry_to_dd(
        detail::pow_tables::pow10_table[k - detail::_dd::pow10_dd_min_exponent]);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::round_nearest_even(const fdd_s& a)
{
    return detail::_dd::round_nearest_even(a);
}

[[nodiscard]] BL_FORCE_INLINE constexpr long detail::_dd_impl::lround_nearest_away_from_zero(
    const fdd_s& x)
{
    long out = 0;
    if (detail::_dd::try_round_to_signed_integer(x, false, out))
        return out;

    return to_signed_integer_or_zero<long>(detail::_dd::round_nearest_away_from_zero(x));
}

[[nodiscard]] BL_FORCE_INLINE constexpr long long detail::_dd_impl::llround_nearest_away_from_zero(
    const fdd_s& x)
{
    long long out = 0;
    if (detail::_dd::try_round_to_signed_integer(x, false, out))
    ...
    return to_signed_integer_or_zero<long long>(detail::_dd::round_nearest_away_from_zero(x));
}

// arithmetic and comparisons
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::fma(const fdd_s& x, const fdd_s& y, const fdd_s& z)
{
    if (detail::fp::isinf_or_nan(x.hi) || detail::fp::isinf_or_nan(y.hi) || detail::fp::isinf_or_nan(z.hi)) [[unlikely]]
        return fdd_s{ std::fma(x.hi, y.hi, z.hi), 0.0 };

    const double leading_product = x.hi * y.hi;
    if (!detail::fp::isfinite(leading_product)) [[unlikely]]
        return detail::_dd::fma_overflow_scaled(x, y, z);

    return detail::_dd::fma_finite_dispatch(x, y, z, leading_product);
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::fmin(const fdd_s& a, const fdd_s& b)
{
    ...
    if (a.hi == 0.0 && a.lo == 0.0)
        return detail::_dd::signbit(a.hi) ? a : b;

    return a;
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::fmax(const fdd_s& a, const fdd_s& b)
{
    if (detail::fp::isnan(a.hi)) [[unlikely]]
        return b;
    if (detail::fp::isnan(b.hi)) [[unlikely]]
    ...
    if (a.hi == 0.0 && a.lo == 0.0)
        return detail::_dd::signbit(a.hi) ? b : a;

    return a;
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::fdim(const fdd_s& x, const fdd_s& y)
{
    if (!detail::fp::isinf_or_nan(x.hi) && !detail::fp::isinf_or_nan(y.hi))
    {
        const bool x_greater_y = (x.hi > y.hi) || (x.hi == y.hi && x.lo > y.lo);
        return x_greater_y ? sub_finite_inline(x, y) : fdd_s{ 0.0 };
    }
    ...
    if (isinf(x))
    {
        if (signbit(x))
            return fdd_s{ 0.0 };
        return (isinf(y) && !signbit(y)) ? fdd_s{ 0.0 } : std::numeric_limits<fdd_s>::infinity();
    }
    if (isinf(y))
        return signbit(y) ? std::numeric_limits<fdd_s>::infinity() : fdd_s{ 0.0 };

    return (x > y) ? sub_finite_inline(x, y) : fdd_s{ 0.0 };
}

[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::copysign(const fdd_s& x, const fdd_s& y)
{
    return detail::_dd::signbit(x.hi) == detail::_dd::signbit(y.hi) ? x : -x;
}

// remainders
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::fmod(const fdd_s& x, const fdd_s& y)
{
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::remquo(const fdd_s& x, const fdd_s& y, int* quo)
{
    using namespace detail::_dd;
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::modf(const fdd_s& x, fdd_s* iptr) noexcept
{
    if (isnan(x))
    {
        if (iptr)
    ...
    const fdd_s i = detail::_dd_impl::trunc(x);
    if (iptr)
        *iptr = i;

    fdd_s frac = sub_finite_inline(x, i);
    if (iszero(frac))
        frac = detail::_dd::signed_zero(signbit(x));
    return frac;
}

// decomposition and scaling
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::ldexp(const fdd_s& x, int e)
{
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::frexp(const fdd_s& x, int* exp) noexcept
{
    if (exp)
        *exp = 0;
    ...
    if (hi_exponent != 0u && hi_exponent != 0x7ffu)
    {
        ...
        return fdd_s{ scaled_hi, scaled_lo };
    }
    else
    {
    ...
    fdd_s m{hi, (scaled_hi - hi) + scaled_lo};
...
[[nodiscard]] BL_FORCE_INLINE constexpr int detail::_dd_impl::ilogb(const fdd_s& x) noexcept
{
    if (isnan(x))
        return FP_ILOGBNAN;
    if (iszero(x))
...
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::logb(const fdd_s& x) noexcept
{
    if (isnan(x))
        return x;
    if (iszero(x))
    ...
    return fdd_s{ static_cast<double>(ilogb_finite_fast(x)), 0.0 };
}

// adjacent values
[[nodiscard]] BL_FORCE_INLINE constexpr fdd_s detail::_dd_impl::nextafter(const fdd_s& from, const fdd_s& to) noexcept
{
    ...
    if (iszero(from))
        return signbit(to)
            ? fdd_s{ -std::numeric_limits<double>::denorm_min(), 0.0 }
            : fdd_s{  std::numeric_limits<double>::denorm_min(), 0.0 };
    if (isinf(from))
        return signbit(from)
            ? -std::numeric_limits<fdd_s>::max()
            :  std::numeric_limits<fdd_s>::max();
    ...
    const double step = detail::fp::nominal_ulp_step(
        ...
        std::numeric_limits<fdd_s>::digits);
