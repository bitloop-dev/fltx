...
| fqd expression lifetime, routing and materialization | `contracts/expressions.cpp` | direct conversion matrix in `constexpr/core.cpp` | Covers delayed temporaries and storage members, every node shape, explicit conversion to all fundamental arithmetic targets, eager-result equivalence, independent expected values, and a fusion-sensitive residual |
