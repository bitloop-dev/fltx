...
namespace fltx::tests::accuracy
{
    namespace
    {
        ...
        [[nodiscard]] std::vector<domain> trig(std::size_t n)
        {
            std::vector<domain> out;
            out.push_back(domains::moderate(n));
            out.push_back(domains::argument_reduction_dd(n));
            out.push_back(
                trig_domains::quadrant_boundaries<bl::fdd>(n));
            return out;
        }
        ...
        [[nodiscard]] domains::binary_domain fmod_quotient_reduction_inputs()
        {
            ...
            const bl::fdd decimal_lhs = bl::parse<bl::fdd>(
                "4.6958550912494028428400315673292717414e+19");
            const bl::fdd decimal_rhs = bl::parse<bl::fdd>(
    ...
    {
        using Float = bl::fdd;
        runner<bl::fdd> run(output, settings);
        ...
        run.unary_exact(
            ...
            [](auto x) { return bl::fdd{static_cast<std::int64_t>(bl::lround(x))}; },
            [](const real& x) {
        ...
        run.unary_exact(
            ...
            [](auto x) { return bl::fdd{static_cast<std::int64_t>(bl::llround(x))}; },
            [](const real& x) {
                const real rounded = mpfr::round_away_from_zero(x);
                return rounded == 0 ? real{0} : rounded;
            },
        ...
        run.binary_pair(
            ...
            [](auto x, auto y) {
                int quotient = 0;
                const bl::fdd remainder = bl::remquo(x, y, &quotient);
                const int low_bits = quotient < 0 ? -((-quotient) & 7) : (quotient & 7);
                return std::pair{remainder, bl::fdd{static_cast<double>(low_bits)}};
            },
            [](const real& x, const real& y) {
        ...
        run.unary_pair_exact(
            ...
            [](auto x) {
                bl::fdd integral{};
                const bl::fdd fractional = bl::modf(x, &integral);
        ...
        run.unary_pair_exact(
            ...
            [](auto x) {
                int exponent = 0;
                const bl::fdd fraction = bl::frexp(x, &exponent);
                return std::pair{fraction, bl::fdd{static_cast<double>(exponent)}};
            },
            [](const real& x) {
        ...
        run.unary_exact(
            ...
            [](auto x) { return bl::fdd{static_cast<double>(bl::ilogb(x))}; },
